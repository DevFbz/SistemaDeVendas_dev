...
class DashboardTab(QWidget):
    ...
    def refresh(self):
        ...
        for r,s in enumerate(recent):
            self.st.setItem(r,0,QTableWidgetItem(s.get("date","")[:16].replace("T"," ")))
...
class CustomersTab(QWidget):
    ...
    def _hist(self):
        ...
        for r,s in enumerate(sorted(sales,key=lambda x:x.get("date",""),reverse=True)):
            t.setItem(r,0,QTableWidgetItem(s.get("date","")[:16].replace("T"," ")))
...
class HistoryTab(QWidget):
    def __init__(self, dm, parent=None):
        super().__init__(parent); self.dm=dm; self._sales=[]; self._build(); self.refresh()
    def _build(self):
        lay = QVBoxLayout(self); lay.setContentsMargins(20,20,20,20); lay.setSpacing(12)
        lay.addWidget(SectionTitle("  Historico de Vendas"))
        tb = QHBoxLayout()
        self.srch = SearchBar("Cliente ou ID..."); self.srch.search_changed.connect(self.refresh); tb.addWidget(self.srch,2)
        tb.addWidget(QLabel("De:"))
        self.df = QDateEdit(QDate.currentDate().addDays(-30)); self.df.setCalendarPopup(True); self.df.setDisplayFormat("dd/MM/yyyy"); tb.addWidget(self.df)
        tb.addWidget(QLabel("Ate:"))
        self.dt = QDateEdit(QDate.currentDate()); self.dt.setCalendarPopup(True); self.dt.setDisplayFormat("dd/MM/yyyy"); tb.addWidget(self.dt)
        bf = QPushButton("  Filtrar"); bf.clicked.connect(self.refresh); tb.addWidget(bf); tb.addStretch(); lay.addLayout(tb)
        self.tbl = QTableWidget(0,7); self.tbl.setHorizontalHeaderLabels(["Data/Hora","Cliente","Itens","Subtotal","Desconto","Total","Pagamento"])
        self.tbl.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        self.tbl.horizontalHeader().setSectionResizeMode(1,QHeaderView.Stretch)
        self.tbl.setEditTriggers(QAbstractItemView.NoEditTriggers); self.tbl.setAlternatingRowColors(True)
        self.tbl.setSelectionBehavior(QAbstractItemView.SelectRows); self.tbl.verticalHeader().setVisible(False)
        self.tbl.doubleClicked.connect(self._detail); lay.addWidget(self.tbl,1)
        self.info = QLabel(); self.info.setObjectName("subtitle"); lay.addWidget(self.info)
    def refresh(self):
        d0=self.df.date().toString("yyyy-MM-dd"); d1=self.dt.date().toString("yyyy-MM-dd")+"T23:59:59"
        txt=self.srch.edit.text().lower()
        sales=[s for s in self.dm.get_sales() if d0<=s.get("date","")<=d1]
        if txt: sales=[s for s in sales if txt in s.get("customer_name","").lower() or txt in s.get("id","").lower()]
        self._sales=sorted(sales,key=lambda x:x.get("date",""),reverse=True)
        self.tbl.setRowCount(len(self._sales)); tot=0
        for r,s in enumerate(self._sales):
            tot+=s.get("total",0)
            self.tbl.setItem(r,0,QTableWidgetItem(s.get("date","")[:16].replace("T"," ")))
            ...
            ti=QTableWidgetItem(fmtR(s.get("total",0))); ti.setForeground(QColor("#00C853")); self.tbl.setItem(r,5,ti)
            self.tbl.setItem(r,6,QTableWidgetItem(s.get("payment_method","--"))); self.tbl.setRowHeight(r,34)
        self.info.setText(f"{len(self._sales)} venda(s) | Total do periodo: {fmtR(tot)}")
    def _detail(self):
        r=self.tbl.currentRow()
        if r<0 or r>=len(self._sales): return
        s=self._sales[r]; dlg=QDialog(self); dlg.setWindowTitle(f"Venda #{s.get('id','')[:8]}"); dlg.setMinimumSize(560,400)
        l=QVBoxLayout(dlg); l.setContentsMargins(16,16,16,16); l.setSpacing(10)
        l.addWidget(QLabel(f"<b>Data:</b> {s.get('date','')[:16].replace('T',' ')}  <b>Cliente:</b> {s.get('customer_name','--')}  <b>Pagamento:</b> {s.get('payment_method','--')}"))
        t=QTableWidget(len(s.get("items",[])),5); t.setHorizontalHeaderLabels(["Codigo","Produto","Qtd","Preco","Total"])
        t.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); t.setEditTriggers(QAbstractItemView.NoEditTriggers); t.verticalHeader().setVisible(False)
        for ri,it in enumerate(s.get("items",[])):
            tot=it["quantity"]*it["unit_price"]*(1-it.get("discount",0)/100)
            ...
            ti=QTableWidgetItem(fmtR(tot)); ti.setForeground(QColor("#00C853")); t.setItem(ri,4,ti)
        l.addWidget(t)
        sl=QHBoxLayout(); sl.addStretch()
        sl.addWidget(QLabel(f"Sub: {fmtR(s.get('subtotal',0))}  Desc: -{fmtR(s.get('discount_value',0))}  <b>TOTAL: {fmtR(s.get('total',0))}</b>"))
        l.addLayout(sl)
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        self.t_hist=HistoryTab(self.dm); self.t_cust=CustomersTab(self.dm)
